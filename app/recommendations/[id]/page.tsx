...
export default function RecommendationDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const foundRecommendation = recommendationsData.find((item) => item.id === id)
  ...
  return (
    <motion.div className="p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <button onClick={() => router.push("/recommendations")} className="flex items-center text-sky-500 mb-4">
      ...
      <h1 className="text-2xl font-bold text-sky-500 mb-4">{recommendation.title}</h1>
